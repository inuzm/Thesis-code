...
cd lda
for d in *; do mv "$d" "`echo $d | sed 's/data/tweets/'`"; done
cd ..

echo "Fitting Hierarchical Dirichlet Process"

python ../Python\ dependencies/online-hdp-master/run_online_hdp.py \
--D 1459 --W 2369 --tau 64 --kappa 0.6 --batchsize 256 \
--max_time 21600 --var_converge 1e-8 --corpus_name tweets \
--data_path ./lda/tweets.dat --directory . --save_lag 10000000 \
--alpha 1.0 --gamma 0.1 --eta 0.01 --T 20 --K 5

echo "Getting the words for all topics"

cd tweets-kappa-0.6-tau-64-batchsize-256
python ../../Python\ dependencies/onlineldavb-master/printtopics.py \
../lda/tweets.vocab ./final.topics > tweets-kappa-0.6-tau-64-batchsize-256.txt

echo "Splitting big file into a file by topic"

awk -v RS= '{print > ("topic-" NR ".txt")}{close("topic-" NR ".txt")}' \
tweets-kappa-0.6-tau-64-batchsize-256.txt

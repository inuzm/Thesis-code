...
cd lda
for d in *; do mv "$d" "`echo $d | sed 's/data/news/'`"; done
cd ..

echo "Fitting Hierarchical Dirichlet Process"

python ../Python\ dependencies/online-hdp-master/run_online_hdp.py \
--D 30436 --W 8668 --tau 64 --kappa 0.6 --batchsize 1024 \
--max_time 21600 --var_converge 1e-8 --corpus_name news \
--data_path ./lda/news.dat --directory . --save_lag 10000000 \
--alpha 1.0 --gamma 0.1 --eta 0.01 --T 50 --K 20

echo "Getting the words for all topics"

cd news-kappa-0.6-tau-64-batchsize-1024
python ../../Python\ dependencies/onlineldavb-master/printtopics.py \
../lda/news.vocab ./final.topics > news-kappa-0.6-tau-64-batchsize-1024.txt

echo "Splitting big file into a file by topic"

awk -v RS= '{print > ("topic-" NR ".txt")}{close("topic-" NR ".txt")}' \
news-kappa-0.6-tau-64-batchsize-1024.txt
